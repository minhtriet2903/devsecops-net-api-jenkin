...
namespace WebApplication1.Controllers
{
    ...
    public class EmployeesController : ControllerBase
    {
        ...
        public async Task<ActionResult<Employee>> GetEmployeeById(int id)
        {
            try
            {
                var employee = await _employeeService.SelectEmployee(id);
                return Ok(employee);
            }
            catch (Exception exp)
            {
        ...
        public async Task<ActionResult> AddEmployee([FromBody] Employee employee)
        {
            try
            {
                ...
                if (employee != null)
                {
                    result = await _employeeService.SaveEmployee(employee);
                }
                return Ok(result);
            }
            catch (Exception exp)
        ...
        public async Task<ActionResult> EditEmployee(int id, [FromBody] Employee employee)
        {
            ...
            {
                string result = string.Empty;
                result = await _employeeService.UpdateEmployee(id, employee);
                return Ok(result);
            }
            catch (Exception exp)
            {
